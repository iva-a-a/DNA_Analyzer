...
#include "../../../cli/commands/AlignCommandHandler.hpp"
#include "../../../core/common/errors/Errors.hpp"

namespace {

class AlignCommandHandlerTest : public ::testing::Test {
protected:
  AlignCommandHandler handler;
...
TEST_F(AlignCommandHandlerTest, ThrowsWhenNoArguments) {
  const CommandLineOptions options{CommandType::Align, {}};

  EXPECT_THROW(handler.run(options), InputFormatError);
}

TEST_F(AlignCommandHandlerTest, ThrowsWhenTooManyArguments) {
...
TEST_F(AlignCommandHandlerTest, PrintsFullAlignmentAndReturnsZero) {
  const auto inputPath = makeFilePath("align_command.txt");
